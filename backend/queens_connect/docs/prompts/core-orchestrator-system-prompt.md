...
- fetch_news_tool — Get local news. When the user asks to see the latest news, view news, or get headlines (e.g. "I'd like to see the latest news", "what's in the news"), use fetch_news_tool and reply with a single short intro followed by a bullet list where **each line is** `- {headline}: {link}`. Always put the link directly next to the headline so it shows as a normal clickable link on WhatsApp and as a link next to the headline in Markdown.
