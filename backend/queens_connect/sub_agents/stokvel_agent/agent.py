"""Stokvel sub-agent: create stokvel, list stokvels (public info only), join stokvel, first payment link."""
...
def _load_instruction() -> str:
    path = getattr(config, "STOKVEL_AGENT_PROMPT_PATH", None) or (
        config.REPO_ROOT / "docs" / "prompts" / "stokvel-agent.md"
    )
    if path.exists():
        text = path.read_text(encoding="utf-8")
        text = text.replace("{waNumber}", "{waNumber?}").replace("{currentDate}", "{currentDate?}")
        # Escape literal placeholders so ADK does not treat them as context variables
        text = (
            text.replace("{name}", "[stokvel name from create_stokvel_tool]")
            .replace("{accessToken}", "[access token from create_stokvel_tool]")
            .replace("{stokvelName}", "[stokvel name]")
        )
        return text
...
def _default_instruction() -> str:
    return """You are the Stokvel agent for Queens Connect.

**Create stokvel:** When the user wants to create a stokvel, get: name, about (short description), and monthly contribution fee (in Rands). Call create_stokvel_tool(owner_wa_number=waNumber from state, name, about, monthly_contribution_cents). Parse amounts like "R200" or "200 rand" to cents (20000). Reply with: "Stokvel **[stokvel name from create_stokvel_tool]** has been created. Your **access token** is `[access token from create_stokvel_tool]` — keep it safe. Use it when you want to check stokvel balances or request a payout."
...
4. Call create_stokvel_contribution_payment_link_tool(stokvel_id, member_wa_number=waNumber, amount_cents=monthlyContributionCents from the add result, description="First contribution – [stokvel name]").
5. Reply: "You're added to **[stokvel name]**. Pay your first contribution here: [payment link]. Reply DONE when you've paid."
...
stokvel_agent = LlmAgent(
    name="stokvel_agent",
    model=config.get_sub_agent_model(),
    description="Create a stokvel, list stokvels (name, about, monthly fee only), or join a stokvel and get the first-contribution payment link.",
