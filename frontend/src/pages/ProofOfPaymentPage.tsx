  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch(
        `${API_URL}/api/lending/loans/${encodeURIComponent(loanId)}/proof`,
        {
          method: 'POST',
          body: formData,
        },
      ...
      if (!res.ok) {
        ...
        const msg = Array.isArray(d)
          ? d[0]?.msg || d[0]?.loc?.join(' ') || 'Upload failed.'
          : d || data.error_message || 'Upload failed.';
      ...
      <div className='w-full max-w-md rounded-2xl bg-white shadow-sm border border-gray-200 p-4 sm:p-6 md:p-8'>
        <div className='text-center mb-4 sm:mb-6'>
          ...
          <p className='mt-2 text-sm text-gray-600'>
            You’ve sent the loan money. Upload a photo or PDF of your proof
            (e.g. transfer screenshot).
          </p>
          {loanId && (
        ...
        {!loanId ? (
          <div className='text-center text-gray-500 text-sm'>
            ...
            <p className='mt-2'>
              Use the link shared with you in WhatsApp (e.g.
              …/pop/&lt;loanId&gt;).
            </p>
            <Link
              to='/'
